<template>
  <div class="space">
    <div class="box">
      ...
      <p>申请时间：<span>{{datas.applyDate|getTimeStamp}}</span></p>
    </div>
    <div class="box">
      <h4 class="recode">病历描述</h4>
      <p class="font28">{{datas.requirement}}</p>
    </div>
    <div class="box box2">
    ...
    <div class="btnBox">
      <oklife-button v-show="datas.cancelFlag==1" @click="submitComplete"  height="2.347rem" width="16.8rem" color="#4776e5">取消申请</oklife-button>
      <a v-show="datas.cancelFlag==2" disabled  class="grayBtn">取消申请</a>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      api.getApplyInfo({params:{appId:_this.appId}}).then(res=>{
        if(res==undefined){
          return false
        }else if(res.code==0){
    ...
    methods: {
      submitComplete: function () {
        this.$router.push({name:'accompanycancelApply',params:{id:this.appId}})
      }
    },
    components: {
      oklifeButton,Toast
