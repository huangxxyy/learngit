<template>
  <div class="address">
    <div class="marginB30 marginT10">
      <div class="textarea">
        <textarea placeholder="请您填写是否结婚？已婚者结婚年龄，结婚次数、是否近亲结婚？女性的孕产次数（包括足月产、早产、流产）、孕产年龄、分娩方式（顺产、剖宫产等）、有无产后感染及产后大出血、子女数量、配偶及子女的健康情况" maxlength="500" @input="descInput"  rows="6" v-model="marriagehistory"></textarea>
        <i class="inputclear" v-if="marriagehistory?true:false" @click="handleClear"></i>
        <span>{{remnant}}字</span>
      </div>
    </div>
    <div class="btnBox">
  ...
  export default {
    ...
    data () {
      return {
        remnant: 500,
        currentValue:false,
        marriagehistory: this.$store.getters['baseInfo/gomarriagehistory']=="请填写"?"":this.$store.getters['baseInfo/gomarriagehistory'],
      }
    },
    computed: {
      patientId(){
    ...
    methods: {
      submit: function() {
        var verify=this.marriagehistory
        ...
        if(verify==""){
          ...
          this.$store.commit("baseInfo/changebaseInfo",{marriagehistory:this.marriagehistory})
          this.$router.push({name:"baseInfo",params:{id:this.patientId}})
        }
      },
      descInput(){
        var txtVal = this.marriagehistory.length;
      ...
      cancel(){
        this.$store.commit("baseInfo/changebaseInfo",{marriagehistory:'暂无'})
        this.$router.push({name:"baseInfo",params:{id:this.patientId}})
      }
    },
    components: {
