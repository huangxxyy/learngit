<template>
  <div class="space">
    <div class="box">
      ...
      <p>年龄：<span>{{datas.age}}</span></p>
      <p>性别：<span>{{datas.sex==1?'男':'女'}}</span></p>
      <p>疾病类型：<span v-for="disName in datas.diseases">{{disName.disName}}</span></p>
    </div>
    <div class="box">
      <h4 class="recode">病历描述</h4>
      <p class="font28">{{datas.problemDesc}}</p>
    </div>
    <div class="box box2">
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      let _this=this
      api.getquestionInfo({params:{appId:_this.appId}}).then(res=>{
        if (undefined == res) {
          return false
        } else if (0 == res.code) {
