<template>
  <div class="name">
    <div class="marginB30">
      <oklife-input placeholder="请输入您的腰围" type="number" v-model="waistline" oninput="if(value.length>5)value=value.slice(0,5)"></oklife-input>
...
<script>
  ...
  import oklifeButton from '../../../src/components/button'
      ...
      name:'waistline',
        data () {
            return {
              waistline: this.$store.state.baseInfo.baseInfo.waistline
            }
        },
        computed:{
          patientId(){
        ...
        methods:{
          submit: function() {
            if(this.waistline=="" ) {
              let instance = Toast('请填写您的腰围');
              setTimeout(() => {
                instance.close();
              }, 2000)
            }else {
              this.$store.commit("baseInfo/changebaseInfo",{waistline:this.waistline,})
