<template>
  <div class="name">
    <div class="marginB30">
      <oklife-input placeholder="请输入您的腹围" type="number" v-model="paunch" oninput="if(value.length>5)value=value.slice(0,5)"></oklife-input>
...
<script>
  ...
  import oklifeButton from '../../../src/components/button'
      ...
      name:'paunch',
        data () {
            return {
              paunch: this.$store.state.baseInfo.baseInfo.paunch
            }
        },
        computed:{
          patientId(){
        ...
        methods:{
          submit: function() {
            if(this.paunch=="" ) {
              let instance = Toast('请填写您的腹围');
              setTimeout(() => {
                instance.close();
              }, 2000)
            }else {
              this.$store.commit("baseInfo/changebaseInfo",{paunch:this.paunch,})
