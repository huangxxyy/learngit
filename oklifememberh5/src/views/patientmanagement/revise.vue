  ...
  <div class="patientinfo">
    ...
    <div class="datapickerBox" v-show="age_flag">
      <mt-datetime-picker class="datapicker" :visibleItemCount="itemCount" v-model="defaultIndex"
                          type="date" v-bind:start-date='startDate' v-bind:end-date="endDate" v-bind:closeOnClickModal="closeOnClickModal"
  ...
  export default {
    data () {
      return {
        ...
        actions: [{name: "本人", method: this.chooserelationship}, {
          name: "父母",
          method: this.chooserelationship
        }, {name: "子女", method: this.chooserelationship}, {name: "亲属", method: this.chooserelationship}],
        byDefault: this.$store.getters['patientinfo/getbyDefault'],
    ...
    computed: {
      ...
      patientId(){
          return this.$store.state.patientinfo.patientinfo.patientId
      },
    ...
    beforeRouteEnter(to, from, next){
      next(vm => {
        ...
        if (from.path == "/managementList/patientname" || from.path == "/managementList/patientPhone" || from.path == "/managementList/patientIdcard"|| from.path == "/managementList/patientinsurance"|| from.path == "/managementList/patiEmrg"|| from.path == "/managementList/patiEmrgPhone") {
          return;
        } else {
          vm.$store.dispatch('patientinfo/getpatientinfo',{params:{"patientId":vm.$route.params.id}})
          console.log(vm.$store.getters['patientinfo/getbyDefault']);
        };
      })
    },
      ...
      submit(){
        ...
        if (this.patientName == "请填写真实姓名") {
          ...
          setTimeout(() => {
            ...
            let _this=this;
            _this.$store.dispatch("patientinfo/patiEmrgPhone", params)
        }
      },
    },
    mounted () {
