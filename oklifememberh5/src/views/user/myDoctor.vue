<template>
  <div class="space">
    ...
    <div class="nothing" v-if="datas.length==0">
      ...
      <p class="addparientP">您还没有私人医生<br /><span>享受平台服务就可以拥有私人医生</span></p>
      <router-link to="" class="enjoy">享受服务</router-link> <!---todo 此处点击回到首页--->
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      getDoctor(data,callback){//获取专家列表
        ...
        api.getMydoctor({params:{"id":this.userId}},{data:data}).then(res => {
          if(res==undefined){
            return false
          }else if (0 == res.code) {
            callback(res)
