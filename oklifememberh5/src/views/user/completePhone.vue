<template>
  <div class="bindmobile">
    <div class="contain">
      ...
      <div class="containcell">
        <input type="number" class="inputcell inputnum" placeholder="请输入验证码" ref="inputcode" v-model="code" oninput="if(value.length>11)value=value.slice(0,11)">
        <div class="left">
  ...
  export default {
    ...
    data () {
      return {
        ...
        isshownum:true,
        isshowphone:true,
        cellPhone:this.$store.getters['completeInfo/toTel']=="用于接收就诊信息，请认真填写"?"":this.$store.getters['completeInfo/toTel']
      }
    },
    computed: {
      completeuserId(){
...
//        alert(document.getElementsByTagName('title')[0].text);
    ...
    watch: {
      cellPhone(){
        ...
        }else if(this.cellPhone=="用于接收就诊信息，请认真填写"){
          this.cellPhone=""
        } else {
          this.clearshowphone = false
        }
        if (validatorUtils.isMobile(this.cellPhone)) {
    ...
    methods: {
      ...
      getVerifiCode(){
        ...
        alert('已经发送验证码');
        this.$store.dispatch("completeInfo/geticode", {params:{"cellphone": this.cellPhone}})
        let _this = this;
        setTimeout(function () {
      ...
      submitphone(){
        if (this.cellPhone == '') {
          ...
          api.identifyIdCode({data:{cellphone: this.cellPhone,
            ...
            if(res.code==0) {
              this.$store.commit('completeInfo/changcompleteInfo',{cellPhone:this.cellPhone});
...
<style lang="scss" scoped>
  ...
  .containcell:last-child{border-bottom:none;}
  .inputcell {
    border: none;
    height: px2rem(100);
    font-size: px2rem(28);
