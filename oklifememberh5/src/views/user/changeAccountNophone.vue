  ...
  <div class="bindmobile">
    <div class="contain">
      <div class="containcell">
        <input type="number" class="inputcell inputcellphone" placeholder="请输入手机号" onkeypress="return (/[\d]/.test(String. fromCharCode(event.keyCode)))" oninput="if(value.length>11)value=value.slice(0,11)" ref="inputphone" v-model="cellphone">
        <div class="left">
        ...
        <div class="right">
          <img src="../../assets/clear.png" class="clear" v-if="clearshowphone" @click="clearinput(1)"
               ref="clearimgphone">
        </div>
      </div>
      <hr class="line">
...
//        alert(document.getElementsByTagName('title')[0].text);
    ...
    methods: {
      submitphone(){
        ...
        var vcode=this.code;
        if(!commonutils.isEnNumerFirstEn(inputAccount)){
      ...
      getVerifiCode(){
        ...
        alert('已经发送验证码');
        let _this = this;
        _this.$store.dispatch("userinfo/geticode", {params:{"cellphone": _this.cellphone}})
        setTimeout(function () {
