...
/**
 /* 判断是否是正确日i，不小于最小时间
 * @param strValue 校验的值
 * @param value 第二个参数，最小时间为几天
 * @returns {boolean}
 */
function dateAfterToday (strValue, value) {
  ...
  if (r == null)
    return false
  var d = new Date(r[1], r[3] - 1, r[4])
  var dtody = new Date(new Date().setHours(0, 0, 0, 0))
  console.log(Date.parse(dtody) + (value * 24 * 60 * 60 * 1000))
  console.log(Date.parse(d))

  if (d.getFullYear() == r[1] && (d.getMonth() + 1) == r[3] &&
    d.getDate() == r[4]) {
    return (Date.parse(d) >=
    Date.parse(dtody) + (value * 24 * 60 * 60 * 1000))
  } else {
    return false
  }
}
...
function isDateLongTime (str) {
  ...
  return (d.getFullYear() == r[1] && (d.getMonth() + 1) == r[3] &&
  d.getDate() == r[4] && d.getHours() == r[5] && d.getMinutes() == r[6] &&
  d.getSeconds() == r[7])
}
...
function isCardID (strValue) {
  ...
  if (len == 15) {
    ...
    if (birthday.getFullYear() == year &&
      (birthday.getMonth() + 1) == month && birthday.getDate() == day) {
      //判断年份的范围（3岁到100岁之间)
  ...
  if (strValue.length == 15) {
    ...
    var arrCh = new Array('1', '0', 'X', '9', '8', '7', '6', '5', '4', '3',
      '2')
    var cardTemp = 0, i
    strValue = strValue.substr(0, 6) + '19' +
      strValue.substr(6, strValue.length - 6)
    for (i = 0; i < 17; i++) {
